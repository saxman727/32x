...
static volatile unsigned int vblankCount = 0;

static volatile unsigned short currentFB = 0;

#define UNCACHED_CURFB (*(short *)((int)&currentFB|0x20000000))
#define UNCACHED_VBLANKCOUNT (*(int *)((int)&vblankCount|0x20000000))
...
void HwMdPrintHexValue(int x, int y, int color, unsigned int value, int digits)
{
	char str[digits+1];
	
	for (int i=0; i < digits; i++) {
		unsigned int singleDigit = (value >> (i<<2)) & 0xF;
		singleDigit += (singleDigit < 0xA) ? '0' : 'A'-10;
		str[digits-i-1] = singleDigit;
	}
	str[digits] = '\0';
	
	HwMdPuts(str, color, x, y);
}

void HwMdPrintDecValue(int x, int y, int color, unsigned int value, int digits)
{
	char str[digits+1];
	
	for (int i=0; i < digits; i++) {
		unsigned int singleDigit = value % 10;
		str[digits-i-1] = singleDigit + '0';
		value /= 10;
	}
	str[digits] = '\0';
...
void HwMdPrintFloatValue(int x, int y, int color, unsigned int value, int leftDigits, int rightDigits)
{
	int characters = leftDigits + 1 + rightDigits;
	char str[characters+1];
	
	for (int i=0; i < rightDigits; i++) {
		unsigned int singleDigit = value % 10;
		str[characters-i-1] = singleDigit + '0';
		value /= 10;
	}
	
	str[leftDigits] = '.';
	
	for (int i=0; i < (characters - rightDigits - 1); i++) {
		unsigned int singleDigit = value % 10;
		str[leftDigits-i-1] = singleDigit + '0';
		value /= 10;
	}
